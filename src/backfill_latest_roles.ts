...
async function extractLatestRole(experience: string): Promise<string> {
    ...
    try {
        const completion = await client.chat.completions.create({
            ...
            messages: [
                { role: "system", content: "You are an AI that extracts the most recent job role/title from a candidate's experience text. Return only the job title name, and nothing else. Keep it under 255 characters. Do not include company names or dates." },
...
async function run() {
    ...
    for (let i = 0; i < result.rows.length; i += batchSize) {
        ...
        console.log(`Processing batch ${Math.floor(i/batchSize) + 1} of ${Math.ceil(result.rows.length/batchSize)}...`);
