import { CandidateSink, OutreachHistoryRepository, OutreachNotifier, RiskScorer, ScreenedCandidate, ScreeningStrategy } from '../domain/ports';
...
export class OutreachOrchestrator {
    ...
    private excludeSameCompany(candidates: ScreenedCandidate[], companyName: string): ScreenedCandidate[] {
        const canonical = companyName
            .trim().toLowerCase()
            .replace(GENERIC_SUFFIXES, '').replace(/\s+/g, ' ').trim();
        if (canonical.length < 3) return candidates;

        const tokens = [canonical, ...canonical.split(/\s+/).filter((t) => t.replace(/[^\w\s]/g, '').trim().length >= 4)];
    ...
    public async run(cmd: OutreachCommand): Promise<void> {
        ...
        if (!strategy) {
            logError('orchestrator_missing_strategy', new Error(`No strategy for engine: ${cmd.screening.engine}`));
            return;
        }

        const emailsList = cmd.recipients.split(',')
            .map((e) => e.trim())
            .filter((e) => e !== '' && !BLOCKED_RECIPIENTS.has(e.toLowerCase()));

        let candidates = this.excludeSameCompany(this.normaliseCandidates(cmd.uiCandidates), cmd.companyName);

        if (candidates.length === 0 && cmd.screening.engine === 'llm') return;

        // Dedup Check
        const urls = candidates.map(c => c.profileUrl);
        const alreadySent = (!cmd.bypassDeduplication && emailsList.length > 0)
        ...
        if (notified && emailsList.length > 0) {
            ...
            logWarn('email_failed_history_skipped', { batchId: cmd.batchId, candidates: passedUrls.length });
        }
